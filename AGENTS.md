...
--screen-glow: rgba(0, 255, 0, 0.6); /* Brilho CRT */
```
...
.accent {
  ...
  text-shadow: 0 0 4px var(--screen-glow);
}
.muted {
  color: var(--color-crt-muted);
}
.fg-2 {
  color: var(--color-crt-fg-2);
}
.tabular {
...
- [x] Criar `lib/constants.ts` com dados de navegação e redes sociais
- [x] Criar `lib/utils.ts` com funções utilitárias
- [x] Implementar `BootScreen.tsx` com animação de inicialização
